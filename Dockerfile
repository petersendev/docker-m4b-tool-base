...
FROM alpine:3.9.2 as m4b-builder
...
#
##############################
FROM petersendev/aspnet:5.0 as m4b-base
ENV WORKDIR /mnt/
...
COPY --from=m4b-builder /usr/local/bin/mp4* /usr/local/bin/fdkaac /usr/local/bin/
COPY --from=m4b-builder /usr/local/lib/libmp4v2* /usr/local/lib/
...
# ADD AudiobookManager.M4bProcessor/Dockerfile AudiobookManager.M4bProcessor/dist/m4b-tool.phar* /tmp/
...
#    && echo "M4B_TOOL_PRE_RELEASE_LINK = ${M4B_TOOL_PRE_RELEASE_LINK}" \
